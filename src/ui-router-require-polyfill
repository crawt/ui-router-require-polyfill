...
;(function (angular, undefined) {
	...
	function DecoratorConstructor($delegate, $log, $q, $compile, $controller, $interpolate, $state, $ocLazyLoad) {
		...
		function processTpl(tpl) {
			...
			tpl = tpl.replace(/<link(([\s\S])*?)>/, function (match) {
				let head = angular.element('head').html();
				if(head.indexOf(match) === -1){
					angular.element('head').append(match);
				}
				return '<!-- css replaced -->'
			});
